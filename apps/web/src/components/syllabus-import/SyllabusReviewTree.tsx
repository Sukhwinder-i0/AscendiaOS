...
export function SyllabusReviewTree({ subjects, onChange }: SyllabusReviewTreeProps) {
  ...
  return (
    <div className="space-y-3 bg-slate-50 border border-slate-200 rounded-lg p-4 max-h-[480px] overflow-y-auto">
      <div className="flex items-center justify-between pb-2 border-b border-slate-200">
        ...
        <button
          ...
          className="inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-medium rounded-md bg-blue-50 text-blue-700 border border-blue-200 hover:bg-blue-100 transition-colors"
        >
      ...
      {subjects.length === 0 ? (
        ...
        subjects.map((sub: ExtractedSubject, sIdx: number) => {
          ...
          return (
            <div key={subKey} className="border border-slate-200 rounded-lg bg-white overflow-hidden">
              {/* Subject Row */}
              <div className="flex items-center justify-between px-3 py-2.5 bg-slate-50 hover:bg-slate-100 transition-colors border-b border-slate-200">
                  ...
                  {editingNodeId === subKey ? (
                    <div className="flex items-center gap-1 flex-1">
                      <input
                        ...
                        className="bg-white text-slate-900 text-sm px-2 py-1 rounded border border-blue-500 focus:outline-none w-full"
                        autoFocus
                      />
                      <button onClick={() => saveRename([sIdx])} className="p-1 text-emerald-600 hover:bg-emerald-50 rounded">
                        <Check className="w-4 h-4" />
                      </button>
                      <button onClick={() => setEditingNodeId(null)} className="p-1 text-slate-400 hover:bg-slate-200 rounded">
                        <X className="w-4 h-4" />
                      </button>
                    </div>
                  ) : (
                    <span className="font-semibold text-slate-900 text-sm truncate">{sub.name}</span>
                  )}
                </div>
              ...
              {isSubExpanded && (
                <div className="pl-6 pr-2 py-2 space-y-2">
                  {sub.chapters.length === 0 ? (
                    <div className="text-xs text-slate-400 py-1 pl-2">No chapters in this subject.</div>
                  ) : (
                    sub.chapters.map((chap: ExtractedChapter, cIdx: number) => {
                      ...
                      return (
                        <div key={chapKey} className="border border-slate-200 rounded-md bg-white">
                          {/* Chapter Header */}
                          <div className="flex items-center justify-between px-2.5 py-2 hover:bg-slate-50 transition-colors">
                            <div className="flex items-center gap-2 flex-1 min-w-0">
                              <button
                              ...
                              {editingNodeId === chapKey ? (
                                <div className="flex items-center gap-1 flex-1">
                                  <input
                                    ...
                                    value={editingName}
                                    onChange={(e) => setEditingName(e.target.value)}
                                    className="bg-white text-slate-900 text-xs px-2 py-0.5 rounded border border-blue-500 focus:outline-none w-full"
                                    autoFocus
                                  />
                                  <button onClick={() => saveRename([sIdx, cIdx])} className="p-0.5 text-emerald-600 hover:bg-emerald-50 rounded">
                                    <Check className="w-3.5 h-3.5" />
                                  </button>
                                  <button onClick={() => setEditingNodeId(null)} className="p-0.5 text-slate-400 hover:bg-slate-200 rounded">
                                    <X className="w-3.5 h-3.5" />
                                  </button>
                                </div>
                              ) : (
                                <span className="font-medium text-slate-800 text-xs truncate">{chap.name}</span>
                              )}
                            </div>
                          ...
                          {/* Topics List */}
                          {isChapExpanded && (
                            <div className="pl-5 pr-2 py-1.5 space-y-1 border-t border-slate-100">
                              {chap.topics.length === 0 ? (
                                <div className="text-[11px] text-slate-400 py-0.5 pl-2">No topics.</div>
                              ) : (
                                  ...
                                  return (
                                    <div key={topKey} className="group flex flex-col">
                                      <div className="flex items-center justify-between py-1 px-2 rounded hover:bg-slate-50">
                                        <div className="flex items-center gap-2 flex-1 min-w-0">
                                          {hasSubtopics ? (
                                            ...
                                            <span className="w-3 h-3 text-slate-400 inline-block text-center">•</span>
                                          )}
                                          ...
                                          {editingNodeId === topKey ? (
                                            <div className="flex items-center gap-1 flex-1">
                                              <input
                                                ...
                                                autoFocus
                                              />
                                              <button onClick={() => saveRename([sIdx, cIdx, tIdx])} className="p-0.5 text-emerald-600">
                                                <Check className="w-3 h-3" />
                                              </button>
                                              <button onClick={() => setEditingNodeId(null)} className="p-0.5 text-slate-400">
                                                <X className="w-3 h-3" />
                                              </button>
                                            </div>
                                          ) : (
                                            <span className="text-xs text-slate-700 truncate">{top.name}</span>
                                          )}
                                        </div>
                                      ...
                                      {hasSubtopics && isTopExpanded && (
                                        <div className="pl-6 space-y-1 my-0.5 border-l border-slate-200 ml-3">
                                          {top.subtopics!.map((subtop: { name: string }, stIdx: number) => {
                                            ...
                                            return (
                                              <div
                                                key={subtopKey}
                                                className="group/sub flex items-center justify-between py-0.5 px-2 rounded hover:bg-slate-50"
                                              >
                                                <span className="text-[11px] text-slate-600 truncate">{subtop.name}</span>
