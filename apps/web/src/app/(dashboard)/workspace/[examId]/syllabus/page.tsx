...
export default function SyllabusWorkspacePage() {
  ...
  if (loading) {
    return (
      <div className="text-center py-16">
        ...
        <p className="text-xs text-slate-400">Loading syllabus hierarchy...</p>
      </div>
    );
  }
  ...
  if (error || !tree) {
    return (
      <div className="glass-card p-8 rounded-2xl text-center border border-rose-500/30">
        <p className="text-sm font-semibold text-rose-400">{error || 'Syllabus tree not found.'}</p>
        <button
          onClick={fetchTree}
          className="mt-4 px-4 py-2 bg-slate-800 text-slate-200 text-xs font-semibold rounded-xl hover:bg-slate-700"
        >
          Retry
        </button>
      </div>
